...
namespace GameDataEditor 
{
	public partial class GDEPPX
	{
		...
		public static T GetUnityObject<T>(string key, T defaultValue) where T : UnityEngine.Object
		{
            if (PlayerPrefs.HasKey(key))
            {
		...
		public static List<T> GetUnityObjectList<T>(string key, List<T> defaultValue) where T : UnityEngine.Object
		{
            if (!PlayerPrefs.HasKey(key))
                return defaultValue;

			List<string> paths = GetStringList(key);
		...
		public static List<List<T>> Get2DUnityObjectList<T>(string key, List<List<T>> defaultValue) where T : UnityEngine.Object
		{
            if (!PlayerPrefs.HasKey(key))
                return defaultValue;

			List<List<string>> paths = Get2DStringList(key);
