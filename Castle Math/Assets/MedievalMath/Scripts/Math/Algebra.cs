...
public class Algebra : MonoBehaviour, Question
{
    ...
    public void Start()
    {
        ...
        sub = GameObject.FindObjectOfType<Subtract>();
    ...
    public void GenerateQuestion(int maxDifficulty)
    {
        ...
        int randomQuestionOperand = Random.Range(0, 2); //0 => Add or subtract, 2 => multiply or divide

        if (randomQuestion == 0)
        {
            if (randomQuestionOperand == 0)
            {
                add.GenerateOperands(maxDifficulty);
                string correctAnswer = add.GetCorrectAnswer().ToString();
                questionString = add.GetFirstNum().ToString() + " + " + "X" + " =" + correctAnswer;

                this.correctAnswer = add.GetSecondNum();
                //Set textbox display to formatted question string
                //QuestionText.text = QuestionString;
            }
            else
            {
                sub.GenerateOperands(maxDifficulty);
                string correctAnswer = sub.GetCorrectAnswer().ToString();
                questionString = sub.GetFirstNum().ToString() + " - " + "X" + " =" + correctAnswer;

                this.correctAnswer = sub.GetSecondNum();
                //Set textbox display to formatted question string
                //QuestionText.text = QuestionString;
            }
